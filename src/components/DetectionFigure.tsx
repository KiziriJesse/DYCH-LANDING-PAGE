 ...
 *
 * A dense facial-landmark mesh rendered to canvas. Nodes drift on their own,
 * react to the pointer, and snap into a "lock" state while pressed — the
 * site's one orchestrated motion moment (CLAUDEwebdesign copy.md §6).
 * Fully static under prefers-reduced-motion.
 *
 * Geometry is hand-placed facial topology (cranium, jaw, brows, orbits,
 * nasal ridge, philtrum, lips, cheekbones), not random points, so the mesh
 * reads as a face the way the reference does.
 */
...
/** Proximity mesh — this is what gives the reference its triangulated web. */
...
export function DetectionFigure({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (press > 0.02 && !reduced) {
        ...
        ctx.strokeStyle = `rgba(0, 113, 227, ${0.5 * press})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(6, y);
        ctx.lineTo(W - 6, y);
      ...
      ctx.strokeStyle = `rgba(0, 113, 227, ${0.55 + press * 0.45})`;
      ctx.lineWidth = 1;
